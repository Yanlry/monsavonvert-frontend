    ...
    try {
      ...
      if (userFromLocal) {
        ...
        if (userData && (userData._id || userData.userId)) {
          console.log(
            "✅ ID utilisateur trouvé dans localStorage.user:",
      ...
      if (userFromSession) {
        ...
        if (userData && (userData._id || userData.userId)) {
          console.log(
            "✅ ID utilisateur trouvé dans sessionStorage.user:",
            userData._id || userData.userId
          );
          return userData._id || userData.userId;
        }
      ...
      if (userIdFromLocal) {
        console.log(
          "✅ ID utilisateur trouvé dans localStorage.userId:",
          userIdFromLocal
        );
      ...
      if (userIdFromSession) {
        console.log(
          "✅ ID utilisateur trouvé dans sessionStorage.userId:",
          userIdFromSession
        );
      ...
      if (session_id) {
        ...
        try {
          ...
          if (!pendingOrder || Object.keys(pendingOrder).length === 0) {
            ...
            setError(
              "Données de commande manquantes. Contactez notre service client."
            );
            setLoading(false);
            return;
          }
          ...
          if (!token) {
            console.warn(
              "⚠️ Token non trouvé, l'authentification pourrait échouer"
            );
          }

          // CORRECTION : Utiliser la bonne variable d'environnement Next.js
          // et vérifier que l'URL est définie
          const apiUrl = process.env.NEXT_PUBLIC_API_URL;
          if (!apiUrl) {
            console.error("❌ NEXT_PUBLIC_API_URL non définie dans .env.local");
          ...
          const response = await fetch(`${apiUrl}/api/confirm-order`, {
            method: "POST",
            headers: {
          ...
          if (!response.ok) {
            throw new Error(
              data.error || "Erreur lors de la confirmation de la commande."
            );
          }
        ...
        setTimeout(() => {
          if (!router.query.session_id) {
            console.log(
              "ID de session toujours non trouvé après délai, redirection..."
            );
            router.push("/");
          }
        }, 3000);
    ...
    return (
      <>
        <Head>
          ...
          <meta
            name="description"
            content="Traitement de votre commande chez MonSavonVert"
          />
    ...
    return (
      <>
        <Head>
          ...
          <meta
            name="description"
            content="Erreur lors du traitement de votre commande"
          />
          <link rel="icon" href="/favicon.ico" />
        </Head>
        <div className={styles.successContainer}>
          <div className={styles.successContent}>
            ...
            <p className={styles.noteMessage}>
              Votre paiement a été traité avec succès, mais nous avons rencontré
              un problème lors de l'enregistrement de votre commande. Notre
              équipe a été notifiée et vous contactera sous peu.
            </p>
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Merci pour votre commande chez MonSavonVert"
        />
        <link rel="icon" href="/favicon.ico" />
      </Head>
        ...
        <div className={styles.successContent}>
          ...
          <div className={styles.orderDetails}>
            ...
            <p className={styles.orderInfo}>
              ...
              votre commande. Veuillez également vérifier votre dossier
              courriers indésirables (spam), il est possible que notre message
              s’y trouve par erreur.
            </p>
          </div>
